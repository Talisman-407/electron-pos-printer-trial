...
router.get("/v1/health", (req, res) => {
  res.json({ message: "Hello from Shoopy POS printer App!" });
});
...
router.get("/v1/printer-health", async (req, res) => {
  ...
  if (!printerName) {
    return res
      .status(400)
      .json({ error: "printerName query parameter is required" });
  }
  try {
    const isPrinterConnected = await checkPrinterHealth(
      printerName,
      global.mainWindow
    );
    if (isPrinterConnected) {
      ...
      res
        .status(404)
        .json({ error: `Printer ${printerName} is not connected` });
    }
  } catch (error) {
